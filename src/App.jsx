...
/**
 * Карточка участника (LikerCard)
 * Отвечает за отображение одного пользователя в списке.
 * Стили меняются в зависимости от количества лайков (ранга).
 */
const LikerCard = ({ data, rank }) => {
  const { name, avatar, count } = data;

  // --- КОНФИГУРАЦИЯ СТИЛЕЙ ПО РАНГАМ (TIERS) ---
  ...
  // --- РЕНДЕР: 3-15 МЕСТО (Компактный список) ---
  return (
    <div 
...
export default function App() {
  ...
  // Функция обработки нового лайка
  const handleLike = useCallback((username, avatarUrl, amount = 1) => {
    ...
    if (existingIndex >= 0) {
      const updatedUser = {
        ...
        count: currentUsers[existingIndex].count + amount,
        lastUpdate: Date.now()
      };
      ...
      const newUser = {
        id: username,
        name: username,
        avatar: avatarUrl || 'https://i.pravatar.cc/150?u=default',
        count: amount,
        lastUpdate: Date.now()
      };
    ...
    if (newUsers.length > 50) newUsers.length = 50;
  ...
  useEffect(() => {
    ...
    if (userToConnect) {
      ...
      console.log(`Listening for likes for @${userToConnect}...`);
      
      setTimeout(() => {
        setConnectionStatus('connected');
      }, 1000);

      // Глобальный слушатель для внешних скриптов
      window.onTikTokLike = (username, avatar, count) => {
        handleLike(username, avatar, count);
      };
    } else {
    ...
    <div className="min-h-screen bg-transparent flex flex-col font-sans text-slate-100 overflow-hidden relative">
      ...
      <style>{`
        ...
        @keyframes rgb-border {
          0% { border-color: #22d3ee; box-shadow: 0 0 20px rgba(34,211,238,0.5); }
          33% { border-color: #818cf8; box-shadow: 0 0 20px rgba(129,140,248,0.5); }
          66% { border-color: #e879f9; box-shadow: 0 0 20px rgba(232,121,249,0.5); }
          100% { border-color: #22d3ee; box-shadow: 0 0 20px rgba(34,211,238,0.5); }
        }
        ...
        @keyframes pulse-glow-green {
          ...
          50% { box-shadow: 0 0 50px rgba(132,204,22,0.8); }
        }
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          ...
          {connectionStatus === 'error' && (
             <div className="flex flex-col items-center bg-red-900/40 backdrop-blur-md p-6 rounded-2xl border border-red-500/30">
               <span className="text-red-200 font-bold text-sm">Username missing</span>
               <span className="text-red-300 text-xs mt-1">Add /username to URL</span>
             </div>
          )}
          {connectionStatus === 'connected' && users.length === 0 && (
             <div className="flex flex-col items-center bg-slate-900/20 backdrop-blur-sm p-4 rounded-xl">
                <span className="text-slate-400 font-medium text-xs tracking-widest uppercase animate-pulse">Waiting for first like...</span>
             </div>
          )}
        </div>
      )}

      {/* --- СПИСОК ЛИДЕРОВ (СЛЕВА) --- */}
